...
public interface ConstMethod {
    ...
    default String indexsOld(String t) {
        String index;
        switch (t) {
    ...
    default String typesOld(String t) {
        String type;
        switch (t) {
            case PTConstOld.MIX_TAX:
     ...
     * @return
     */
    default String[] getSearchListColumnOld() {
     ...
     * @return
     */
    default String[] getSearchDetailColumnOld() {
        final String[] columns = {"logo", "name", "qygsDetail.operName", "qygsDetail.registCapi", "qygsDetail.startDate",
                "email", "qygsDetail.address", "qygsDetail.status", "companyId",
    ...
    default List<Kv> converKvOld(Map<String, Long> map) {
    ...
    default List<Kv> converKvOld(Map<String, Long> map, List<TreeBook> list) {
        List<Kv> kvs = new ArrayList<>();
        if (map != null && map.size() > 0)
            for (Map.Entry<String, Long> e : map.entrySet()) {
                Kv kv = new Kv();
                for (TreeBook tb : list) {
                    if (tb.getName().equals(e.getKey())) {
                        kv.setCode(tb.getCode());
                    }
                }
                kv.setKey(e.getKey());
                kv.setVal(String.valueOf(e.getValue()));
