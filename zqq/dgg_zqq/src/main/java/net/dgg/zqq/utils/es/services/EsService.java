package net.dgg.zqq.utils.es.services;
...
public class EsService {
    ...
    public Map<String, Long> filterAggregationCount(AggregationBuilder ab, boolean out,
        ...
        for (Map.Entry<String, String> entry : tmpMap.entrySet())  //组装条件查询，根据不同的数据字典信息，制定处理方式
            ...
            else if (entry.getKey().equals(PTConst.BRAND_TMGOODSSERVICE)) // 规范商品
                orQuery2.must(matchPhraseQuery(entry.getKey(), entry.getValue().substring(entry.getValue().lastIndexOf("_") + 1)));
