...
import net.dgg.zqq.dto.patentsearch.PatentParam;
...
import static net.dgg.zqq.controller.search.CompanyItemController.mixMap;
...
//        logger.info("==========================" + JSON.toJSONString(ap));
        ...
        try {
            putDataMap(startCount, endCount, dataMap, getConditionPatent(ap), mixMap(PTConst.MIX_PATENT, ap.getKeyWord()));
        } catch (Exception e) {
...
//        Assert.isTrue(justLogin(request), PTConst.LOGIN_TWO);
    ...
    private String[] getSearchListColumn() {
        final String[] columns = {"applicationNumber", "abstracts", "piInventName", "legalStatusDesc",
                "inventorString", "applicationDate", "publicationNumber", "piClassifyNum", "iPCDesc", "kindCodeDesc", "keyWord", "publicationDate"};
        return columns;
    }
