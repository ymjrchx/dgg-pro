...
layui.define(function(exports){ //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
  var obj = {
    ztreeInit: function(id,setting,zNodes){
    	$.fn.zTree.init($(id), setting, zNodes);
    },
    getZTreeObj:function(id){
    	$.fn.zTree.getZTreeObj(id);
    }
  };
  layui.config({
	  base: ''+filePath+'/extend/'
  }).extend({ //设定模块别名
      ztree: 'ztree'
  });
  exports('ztree', obj);
}).addcss("ztree/zTreeStyle/zTreeStyle.css","zTreeStyle.css"); 
//引入select2 js
layui.define(function(exports){ //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
  var obj = {
    select2Init: function(id,setting){
    	$(id).select2(setting);
    }
  };
  layui.config({
	  base: ''+filePath+'/extend/' //假设这是你存放拓展模块的根目录
  }).extend({ //设定模块别名
      select2: 'select2'
  });
  exports('select2', obj);
}).addcss("select2/select2.css","select2.css"); 
//下拉树
layui.define(function(exports){ //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
var obj = {
    ztreeSelectInit: function(id,objs){
    	$(id).selectTree(objs)
    }
};
layui.config({
