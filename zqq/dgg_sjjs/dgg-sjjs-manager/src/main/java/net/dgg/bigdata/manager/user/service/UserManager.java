...
import net.dgg.bigdata.manager.common.util.DateUtils;
import net.dgg.bigdata.manager.role.service.RoleService;
import net.dgg.bigdata.manager.user.dao.InitTableDAO;
import net.dgg.bigdata.manager.user.dao.UserRecorderDAO;
...
public class UserManager extends AbstractMongoCacheModule<UserEntity> {
    ...
    public UserManager() {
        ...
        this.setCacheCollectionKey("net.dgg.bigdata.manager.user.service.UserManager");
    }
    ...
    public boolean loadAndSaveUserFromServer(long userId) {
        boolean flag = false;
        HashMap responseData = DggJsonUtils.json2Obj(HttpUtils.sendPost(USERCENTER_USER_APIURL, USERCENTER_APPTOKEN, "userId=" + userId), HashMap.class);
        HashMap userFromServer = DggJsonUtils.json2Obj(responseData.get("dataobject").toString(), HashMap.class);
        UserEntity userEntity = new UserEntity();
        try {
