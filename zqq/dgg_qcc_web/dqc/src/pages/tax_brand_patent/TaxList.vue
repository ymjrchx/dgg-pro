    ...
    <div  style="min-height:700px">
        ...
        <div class="panel panel-default" style="padding:15px 0 45px;">
            ...
            <span class="font-15 text-dark pull-left m-l" v-show="!searching">小顶为您找到
                <span class="text-danger"> {{totalSize}} </span>个符合条件的税号
            </span>
        ...
        <div v-show="!searching">
            <template v-if="totalSize">
                <section v-for="(item,idx) in listArr" :key="idx" class="panel panel-default taxList" >
                    <a  class="list-group-item clearfix brandDetailJump brand_detail">
                        <span class="pull-left thumb-lg m-r">
                            <img :src="item.logo ? item.logo : ''" :onerror="logo" alt="顶企查"> </span>
                        <span class="clear">
                            <!-- <span class="name" @click="goDetail(item.companyId)">{{item.name}}</span> -->
                                <span class="name" @click="goDetail(item.companyId,item.name)" v-html="mathName(item.name)"></span>
                            <small class="text-muted clear text-ellipsis m-t-xs">
                                <span class="taxNum">税号 : {{item.qygsDetail.creditCode || '-'}}</span>
                                <span class="fr table-r">
                                    <span class="nstatus  text-success-lt m-l-xs">{{mathStatus(item.qygsDetail.status)}}</span>
                                </span>
                            </small>
                            <small class="text-muted clear text-ellipsis m-t-xs">
                                地址 : {{item.qygsDetail.address || '-'}}
                            </small>
                        </span>
                    </a>
                </section>
...
export default {
    ...
    methods: {
        ...
        goDetail(companyId,name) {
            // this.$router.push({'path':"/Logo_detail"})
...
.sel-list .panel .name, .ajaxlist .panel .name{
    font-size: 17px;
    font-weight: 700;
}
