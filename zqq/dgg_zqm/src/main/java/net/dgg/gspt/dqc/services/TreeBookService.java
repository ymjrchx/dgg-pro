...
import net.dgg.gspt.dqc.controller.treebook.CommonConstant;
import net.dgg.gspt.dqc.dao.TreeBookDao;
import net.dgg.gspt.dqc.entity.business.treebook.TreeBook;
import net.dgg.gspt.dqc.exception.BookException;
import net.dgg.gspt.dqc.framework.Constants;
...
public class TreeBookService extends AbstractMongoCacheModule<String> {
    ...
    public TreeBook addTreeBook(TreeBook treeBook, Long userId, String userName) {
        ...
        if (treeBookDao.countTreeBookByCode(treeBook.getCode()) > 0) {
            throw new BookException("字典编码已存在");
        }
        Long id = KeyWorker.nextId();
        // 新节点id
        ValidateUtil.notNull(treeBook.getPid(), BookException.class, "父节点pid不可为空");
    ...
    public TreeBook updateTreeBook(TreeBook treeBook, Long userId, String userName) {
        ...
        if (treeBook.getStatus() == 0) {//如果当节点为禁用，需要判断是否有启用的子节点，如果有就不能禁用
            ...
            if (count > 1) {
                throw new BookException("字典下有启用的子级！不能设置禁用");
            }
        }
        //把页面传回来的值放入对象中
        target.setExt1(treeBook.getExt1());
    ...
    public List<TreeBook> getTreeBookListByCode(String code, Integer status, Integer type, Integer level) {
        ...
        if (StringUtils.isEmpty(code)) {
            throw new BookException("编码不能为空");
        }
        if (null == type) {
            throw new BookException("查询类型不能为空");
        }
        if (type.intValue() != 1 && type.intValue() != 2 && type.intValue() != 3) {
            throw new BookException("查询类型只能是1/2/3");
        }
        String cacheName = Constants.TREEBOOK_LIST_KEY + code;
        cacheName += (net.dgg.gspt.dqc.utils.StringUtils.isEmptyObj(type) ? "" : "_type" + type);
        cacheName += (net.dgg.gspt.dqc.utils.StringUtils.isEmptyObj(status) ? "" : "_status" + status);
        cacheName += (net.dgg.gspt.dqc.utils.StringUtils.isEmptyObj(level) ? "" : "_level" + level);

        String treeJson = super.get(cacheName);
        if (!StringUtils.isEmpty(treeJson)) {
            resultList = JsonUtil.json2List(treeJson, TreeBook.class);
        } else {
            resultList = treeBookDao.getTreeBookListByCode(code, status, type, level);
            if (null != resultList && resultList.size() > 0) {
                super.saveToCache(cacheName, JsonUtil.obj2Json(resultList));
            }
        }
        return resultList;
    }
    ...
    public void cacheTreeBook(TreeBook treeBook) {
        ...
        super.saveToCache(Constants.TREEBOOK_OBJ + treeBook.getCode(), JsonUtil.obj2Json(treeBook));
    }
    ...
    public void cacheListTreeBook(String key, List<TreeBook> treeBooks) {
        super.saveToCache(key, JsonUtil.obj2Json(treeBooks));
    }
    ...
    public String getTreeBookName(String code) {
        ...
        if (StringUtils.isEmpty(treeName) || treeName.equals(code)) {
