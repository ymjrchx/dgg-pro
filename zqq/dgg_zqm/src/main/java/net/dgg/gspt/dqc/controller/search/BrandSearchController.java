...
import net.dgg.gspt.dqc.dto.brandsearch.ImgSearchRes2Param;
...
import net.dgg.gspt.dqc.framework.xst.ClientAipOcr;
...
import static net.dgg.gspt.dqc.controller.search.CompanyItemController.indexs;
import static net.dgg.gspt.dqc.controller.search.CompanyItemController.types;
...
//        logger.info("--------------------search list----------" + JSON.toJSONString(ap));
        ...
        putDataMap(condition, startCount, endCount, dataMap, mapConditions(ap), PTConst.F_TRUE, ap); //isphrase设置全局map多条件匹配模式
    ...
    public RestResponse approximate(@RequestBody ApproParam ap) throws Exception {
        Map<String, String> map = mapConditions(ap);
...
//                logger.info(key + "===================" + val);
     ...
     * 数据字典 拦截组装请求参数
     */
    protected static Map<String, String> mapConditions(ApproParam sqp) {
        ...
        if (StringUtils.isNotEmpty(sqp.getGoods())) //商品服务
            condition.put(PTConst.BRAND_TMGOODSSERVICE, sqp.getGoods());
