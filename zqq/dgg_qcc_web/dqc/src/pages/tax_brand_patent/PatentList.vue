<template>
    <div style="min-height:700px">
        ...
        <!-- <section class="panel panel-default taxList" style="line-height:1.8">
            <a  class="list-group-item clearfix brandDetailJump brand_detail" data-key="七夕">
                <span class="clear">
                    ...
                    <small class="text-muted clear text-ellipsis m-t-xs mdtop">
                       <span class="taxNum">申请号 : 91350724MA31XLHH8D</span>
                        <i class="m-l"></i>
                       <span class="taxNum">公布号 : 91350724MA31XLHH8D</span>
                       <span class="fr table-r">
                           <span class="nstatus  text-success-lt m-l-xs">在业</span>
                       </span>
                    </small>
                    <small class="text-muted clear text-ellipsis m-t-xs">
                       <span class="taxNum">外观设计 : 91350724MA31XLHH8D</span>
                        <i class="m-l"></i>
                       <span class="taxNum">申请公布日 : 91350724MA31XLHH8D</span>
                    </small>
                    <small class="text-muted clear text-ellipsis m-t-xs">
                     发明人 : 张三
                    </small>
            ...
            <footer class="panel-footer clear">申请人: <a class="text-primary">奥斯卡机读卡时间段卡视角打开了</a> </footer>
        </section> -->
        <div v-show="!searching">
            <template v-if="totalSize">
                <section v-for="(item,idx) in listArr" :key="idx" class="panel panel-default taxList">
                    <a  class="list-group-item clearfix brandDetailJump brand_detail" @click="goDetail(item.applicationNumber)">
                        <span class="clear">
                            <span class="name" v-html="mathName(item.piInventName)" ></span>
                            <small class="text-muted clear text-ellipsis m-t-xs mdtop">
                            <span class="taxNum">申请号 : {{item.applicationNumber || '-'}}</span>
                                <i class="m-l"></i>
                            <span class="taxNum">公布号 : {{item.publicationNumber || '-'}}</span>
                            <!-- <span class="fr table-r">
                                <span class="nstatus  text-success-lt m-l-xs">{{item.piInventName}}</span>
                            </span> -->
                            </small>
                            <small class="text-muted clear text-ellipsis m-t-xs">
                            <span class="taxNum">申请公布日 : {{item.piApplyAnnounceDate || '-'}}</span>
                            <i class="m-l"></i>
                            <span class="taxNum">类别 :{{item.kindCodeDesc || '-'}}</span>
                            </small>
                            <small class="text-muted clear text-ellipsis m-t-xs">
                            发明人 : {{item.inventorString || '-'}}
                            </small>
                        </span>
                    </a>
                    <footer class="panel-footer clear">申请人: 
                        <a  v-if="item.url" class="text-primary" @click="goCompany(item.url,item.assigneestring)">{{item.assigneestring || '-'}}</a>
                        <span v-else>{{item.assigneestring || '-'}}</span>
                        <!-- <a class="text-primary" @click="goCompany(item.url)">{{item.assigneestring || '-'}}</a>  -->
                        </footer>
                </section>
                <!-- 列表结束-->
                <!-- 分页 开始 -->
...
export default {
    ...
    methods: {
        ...
        goDetail(applicationNumber) {
            this.$router.push({'path':"/Patent_detail",query:{id: applicationNumber}})
        },
         reloadPage(){
            this.pageIndex= 1;
            this.initPage();
