<template>
    <div class="zss-date-picker">
        <input class="zss-date-input" type="text" placeholder="请选择日期" ref='ipt' readonly v-model="currentValue" @focus.stop="inputFocus" />
                ...
                <div class="zss-panel">
                    ...
                    <div class="zss-panel-header">
                        <div class="zss-date-year">
                            ...
                            <div class="zss-date-text zss-text-hover" @click="select='year'">{{firstDate[0]}} 年</div>
                        ...
                        <div class="zss-date-month">
                            ...
                            <div class="zss-date-text zss-text-hover" @click="select='month'">{{firstDate[1]}} 月</div>
                    ...
                    <div class="zss-panel-body">
                        ...
                        <div class="zss-year-box" v-show="select=='year'">
                            <div class="zss-year" v-for="(item,idx) in yearList" @click="clickYear(item)" :key="idx">{{item}}</div>
                        </div>
                        ...
                        <div class="zss-month-box" v-show="select=='month'">
                            <div class="zss-month" v-for="(item,index) in monthArray" @click="clickMonth(index)" :key="index">{{item}}</div>
                        </div>
                        <!--/月-->
                        <!-- 周标题 -->
                        <div class="zss-date-week" v-show="select=='date'">
                            <div class="zss-week" v-for="(item,idx) in weekArray" :key="idx">{{item}}</div>
                        </div>
                        <!-- /周标题 -->
                        <!-- 日期-->
                        <div class="zss-date-box" v-show="select=='date'">
                            <div class="zss-date" :class="{'zss-date-now':isNow(item),'zss-month-now':isMonth(item)}" v-for="(item,index) in dateArray" @click="clickDate(item)" :key="index">{{item[2]}}</div>
                        </div>
                        <!--/日期-->
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        initData() {
            ...
            this.currentDate = (
                this.value || momnet().format("YYYY-MM-DD")
            ).split("-");
            this.firstDate = momnet(this.currentDate.join("-"))
                .startOf("month")
                .format("YYYY-MM-DD")
                .split("-");
            this.getDateArray();
        ...
        getDateArray() {
            ...
            for (var i = 0; i < 42; i++) {
                ...
                if (day == 0) {
                    ...
                    date = momnet(this.firstDate.join("-")).add(
                        i - day,
                        "days"
                    );
                }
        ...
        //点击前一个月
        prevMonth() {
            let prevMonthDate = momnet(this.firstDate.join("-"))
                .startOf("month")
                .add(-1, "month");
        ...
        //点击前一年
        prevYear() {
            let prevYearDate = momnet(this.firstDate.join("-"))
                .startOf("month")
                .add(-1, "year");
        ...
        //点击下一年
        nextYear() {
            let nextYearDate = momnet(this.firstDate.join("-"))
                .startOf("month")
                .add(1, "year");
