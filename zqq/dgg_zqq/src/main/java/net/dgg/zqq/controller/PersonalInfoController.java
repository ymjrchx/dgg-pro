...
import net.dgg.zqq.services.PersonInfoService;
...
public class PersonalInfoController extends BaseController {
    ...
    /**
     * 賬戶安全  信息狀態 1表示已綁定 0表示未綁定
     */

    @RequestMapping(value = "/security", method = RequestMethod.GET)
    @ResponseBody
    @ApiOperation(value = "账户安全信息（1綁定，0未綁定）", notes = "账户安全信息（1綁定，0未綁定）")
    public Object accountSecurity(@ApiParam(name = "userId", value = "用户ID", required = true) @RequestParam String userId) {
        try {
            Map<String, Integer> map = new HashMap<String, Integer>();
            int status = this.personInfoService.findWechatById(userId);//查询微信是否绑定
            int status1 = this.personInfoService.findQQById(userId);//查询QQ是否绑定
            User user = this.personInfoService.findInfoById(userId);
            if (!StringUtils.isNullOrEmpty(user.getUsername())) {
                map.put("username", 1);
            } else map.put("username", 0);
            if (!StringUtils.isNullOrEmpty(user.getEmail())) {
                map.put("email", 1);
            } else map.put("email", 0);
            User user1 = this.userService.findUserById(userId);
            if (!StringUtils.isNullOrEmpty(user1.getLoginPwd())) {
                map.put("loginPwd", 1);
            } else map.put("loginPwd", 0);
            map.put("wechat", status);
            map.put("QQ", status1);
            return this.getSuccessResponse(map);
        } catch (Exception e) {
            e.printStackTrace();
