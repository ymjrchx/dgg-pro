    ...
    public MongoClient init() {
        if (mongoClient == null) {
            if (host.equals("127.0.0.1") || host.equals("192.168.3.90")) {
                ...
                MongoCredential credential = MongoCredential.createScramSha1Credential(StringUtils.isEmpty(username) ? "" : username, StringUtils.isEmpty(username) ? "" : "admin", StringUtils.isEmpty(password) ? "".toCharArray() : password.toCharArray());
