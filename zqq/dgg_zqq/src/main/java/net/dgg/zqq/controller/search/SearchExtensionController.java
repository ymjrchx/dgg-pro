...
import static net.dgg.zqq.controller.search.BrandSearchController.*;
            ...
            try {
                return getSuccessResponse(searchService.dealAggStatus(
                        esService.filterAggregationCount(
                                filterAggregation(LayStatusConstant.extenAggCondition(), PTConst.BRAND_STATE, true),
                                true, mapConditions(ap, "tree"), new HashMap<>(), EsConst.INDEX_BRAND, EsConst.TYPE_COMPANY_BRAND, ap)));
            } catch (Exception e) {
    ...
    public RestResponse topFive(@RequestBody ApproParam ap) {
        ...
        dataMap.put("topFive", searchService.converFiveListMap(
                esService.getListMapBrand(
                        condition, 1, 20, mapConditions(ap, "list"),
                        EsConst.INDEX_BRAND, EsConst.TYPE_COMPANY_BRAND, true, include, exclude, ap)));
