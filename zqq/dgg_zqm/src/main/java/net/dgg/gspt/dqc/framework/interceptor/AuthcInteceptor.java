...
import net.dgg.gspt.dqc.framework.redis.dao.RedisDao;
import net.dgg.gspt.dqc.utils.JsonUtils;
import net.dgg.gspt.dqc.utils.SwapUtils;
...
//        String token = request.getHeader(PTConst.USER_TOKEN);
        ...
        String[] urls = {"/sms/callback", "/order/updateBaoJianStatus", "/webSocketServer", "/sockjs/webSocketServer", "/wechat/callback", "/alipay/callback", "/session/gettoken", "/mobile/userlogin", "/sysuser/login", "/registuser/login", "/imageidentifycode/send", "/sms/sentidentifycode", "/api/", "/journal/recentversion", "/dept/subdept", "task/search", "/registuser/forgetpwd", "/registuser/regist", "/company/recommend_comp", "/regist/loginByCode"};
        for (String url : urls) {
        ...
        if (StringUtils.isEmpty(token) || "null".equalsIgnoreCase(token) || !RedisUtils.exists(token)) {
            checkToken = false;
        }
        if (!checkToken) {
...
//                    }
        ...
        String username = RedisFactory.getJedisCluster().get(token);
