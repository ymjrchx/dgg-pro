        ...
        <div class="sel-bar" style="background:white;">
            <div class="container">
                <div class="top-search-nav clear col-md-9">
                    <div class="pull-left">
                        <a v-for="(item,idx) in styleArr" :key="idx" :class="{'active': idx == isCheck}">{{item}}</a>
                        <!-- <router-link to="/Qualifications_box">商标</router-link> -->
                    </div>
                    <div class="col-md-7 pull-right">
                        <div class="form-group m-b-none">
                            <div class="input-group">
                                <input type="text" class="form-control input-lg" style="font-size: 14px;" name="key" placeholder="请输入你想要查找的信息" autocomplete="off" v-model="keyVal" @keyup.enter.native="submit">
...
export default {
    ...
    watch: {
        $route(to, from) {
            ...
            if (to.path == "/Brand_detail" || to.path == "/Patent_detail")
                this.hasDetail = 1;
            if (from.path == "/Brand_detail" || from.path == "/Patent_detail")
                this.hasDetail = 0;
        }
    },
    created() {
    ...
    methods: {
        initAbout() {
            let obj = {
                keyWord: sessionStorage.getItem("aboutKey") || "顶呱呱",
                count: 5,
                type: "qyn"
            };
            this.$axios({
        ...
        back() {
            ...
            //         break;
            // }
            this.$router.push({ path: "/LawSearch" });
        },
        goIndex() {
        ...
        goSearch() {
            ...
            Common.remberHistory("easy_law", val, 5);
